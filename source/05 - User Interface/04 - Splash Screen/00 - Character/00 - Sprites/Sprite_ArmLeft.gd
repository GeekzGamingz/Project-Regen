...
func check_arm_left():
	...
	sprites_dictionary.sprite_paths.set("sprite_armL", texture.resource_path)
	if sprites_character.server_started:
		var id = multiplayer.get_unique_id()
		var path = texture.resource_path
		rpc("update_network_armL", id, path)
#Update Network Dictionary
@rpc("any_peer", "call_local", "reliable")
func update_network_armL(id, path):
	sprites_dictionary.NETWORK.players[id].set("sprite_armL", path)
