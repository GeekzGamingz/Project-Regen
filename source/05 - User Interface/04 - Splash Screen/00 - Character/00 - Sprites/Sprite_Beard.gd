...
func check_beard():
	if selection_character.is_new:
		...
		match(int(profiles[counter].get("height"))):
			...
			1: offset.y = 0
			2: offset.y = -2.0
	sprites_dictionary.sprite_paths.set("sprite_beard", texture.resource_path)
	if sprites_character.server_started:
		var id = multiplayer.get_unique_id()
		var path = texture.resource_path
		rpc("update_network_beard", id, path)
#Update Network Dictionary
@rpc("any_peer", "call_local", "reliable")
func update_network_beard(id, path):
	sprites_dictionary.NETWORK.players[id].set("sprite_beard", path)
