...
class_name SteamAchievementIcon
extends Sprite2D
## A Steam achievement icon node.
##
## A custom Sprite2D node used to fetch and display a Steam achievement icon based on the API name
## that is set.
##
...
func _ready() -> void:
	if Engine.has_singleton("Steam"):
		Steam.user_achievement_icon_fetched.connect(_on_icon_fetched)
...
func _on_icon_fetched(game_id: int, achievement_name: String, was_achieved: bool, icon_handle: int) -> void:
...
func _set_icon_handle(new_handle: int) -> void:
	...
	if _icon_handle == 0:
		printerr("No icon was found or set for this achievement: %s" % achievement_api_name)
		return
	_load_icon_image()
