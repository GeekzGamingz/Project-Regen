...
const TEMP_FILE = "user://updatekit.zip"
const VERSION_URL: StringName = "https://codeberg.org/godotsteam/godotsteamkit/raw/branch/intro/version"

var api_key_itch: String = "43HDPr26T4Pc1Wdvzst5eahue3Y4fU7TcGT0JXXJ"

var download_location: String = "https://itch.io/api/1/%s/upload/4325039/download" % api_key_itch
...
func set_defaults() -> void:
	downloading.value = 0
	installed_label.text = "Installed version %s" % kit_version
...
func check_for_updates() -> void:
	if not ProjectSettings.get_setting("steam/updates/godotsteamkit/check_for_updates"):
		return
	...
	if http_request.request(VERSION_URL) != OK:
		printerr("Failed to request GodotSteamKit plug-in remote current version")
...
func _on_http_request_completed(result: int, _response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	new_version = str(response)
	if convert_version(new_version) > convert_version(kit_version):
		print("New GodotSteamKit version %s available" % new_version)
		update_label.text = "New version %s" % new_version
		update_button.text = "Download on Itch"
...
func _on_install_pressed() -> void:
	print("Removing older GodotSteamKit %s plug-in" % EditorPlugin.new().get_plugin_version())
	OS.move_to_trash(ProjectSettings.globalize_path("res://addons/godotsteamkit"))
...
# Temporarily open the Itch.io page until I figured out how to download via API.
func _on_update_pressed() -> void:
	OS.shell_open("https://godotsteam.itch.io/godotsteamkit")
	return
	# Restore this later on, sadly.
	...
	var headers: PackedStringArray = [
		...
		"Authorization: Bearer %s" % api_key_itch
		]
	if http_request.request(download_location, headers) != OK:
...
func _on_download_request_completed(result: int, _response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if result != HTTPRequest.RESULT_SUCCESS:
		printerr("Failed to download new GodotSteamKit version %s" % result)
		update_button.disabled = false
		return

	update_label.text = "Update downloaded, asking for confirmation to install"
